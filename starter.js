...
data.map(movieVar => {
  ...
  if (movieVar.id == 4 || movieVar.id == 5)
  {
    let newCast = ["Gab Montano"]
    movieVar.cast= {...movieVar.cast, ...newCast}

    printMovieDetails(movieVar)
  }

  if (movieVar.id == 6){

    let newLanguage = {languages: ["English", "Tagalog"]}
    movieVar = {...movieVar, ...newLanguage}
    printMovieDetails(movieVar)
  }
})
